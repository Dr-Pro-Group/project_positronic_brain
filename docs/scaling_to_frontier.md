...
| **Gradient checkpointing** (`grad_checkpoint`) | ✅ in code | Recomputes the `inner_steps` reverberation in backward; ~½–¼ activation memory, enables larger `inner_steps`/batch. **Note:** automatically disabled (with a warning) when a stateful mechanism — STP, oscillation, homeostasis, or adaptation — is active, because `step` mutates that transient state in place and the backward recompute would evaluate a different function and corrupt the gradient. Use plain BPTT / smaller batch with those flags until the checkpointed region is made state-pure. |
